...
class SiameseCBOW():
    ...
    def __init__(self, input_dim, output_dim, input_length=100, n_positive=2, n_negative=5):
        ...
        with tf.device('/device:GPU:0'):
            ...
            cos_p = [merge([ave_s, l], mode=lambda x: cossim(x), output_shape=(1,), name='p_cos_sim_%d'%i) for i, l in enumerate(ave_s_p)]
            cos_n = [merge([ave_s, l], mode=Lambda(lambda x: cossim(x)), output_shape=(1,), name='n_cos_sim_%d'%i) for i, l in enumerate(ave_s_n)]
